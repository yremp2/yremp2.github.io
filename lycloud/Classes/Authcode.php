...
class Authcode {
	...
	public static function encode ($str, $key = '', $expiry = 0) {
		return self::encrypt($str, 'ENCODE', $key, $expiry);
	}

	public static function decode ($str, $key = '', $expiry = 0) {
		return self::encrypt($str, 'DECODE', $key, $expiry);
	}
	 ...
	 * $expiry：密文有效期(秒)
	 */
	private static function encrypt ($string, $operation = 'DECODE', $key = '', $expiry = 0) {
...
$en = Authcode::encode($str, $key, 10);
var_dump($en);
$de = Authcode::decode($en, $key);
var_dump($de);
 */